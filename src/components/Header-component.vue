  ...
  <header class="sticky-top shadow-sm" :style="{ background: 'var(--ety-dark-blue)' }">
    <div class="container-fluid d-flex justify-content-between align-items-center py-3 px-4 position-relative">
      ...
      <button class="btn d-md-none text-white" @click="isMenuOpen = !isMenuOpen">
        <svg v-if="!isMenuOpen" xmlns="http://www.w3.org/2000/svg" width="28" height="28" fill="currentColor"
        ...
        <svg v-else xmlns="http://www.w3.org/2000/svg" width="28" height="28" fill="currentColor" class="bi bi-x"
          viewBox="0 0 16 16">
          <path
      ...
      <nav class="d-none d-md-flex gap-4">
        <RouterLink v-for="item in navItems" :key="item.to" :to="item.to" class="nav-link text-white"
      ...
      <transition name="fade">
        <div v-if="isMenuOpen" class="position-absolute top-100 start-0 w-100 py-3 z-3"
          :style="{ backgroundColor: 'var(--ety-dark-blue)' }">
          <div class="d-flex flex-column align-items-start px-4">
            <RouterLink v-for="item in navItems" :key="item.to" :to="item.to" class="nav-link text-white py-2"
              active-class="fw-bold text-warning" @click="isMenuOpen = false">
              {{ item.label }}
            </RouterLink>
          </div>
        </div>
...
const navItems = [
  ...
  { label: 'Asea Distributors', to: '/asea' },
